...
%Emmanuel Anane
%Inst. of Biotechnologie, TU-Berlin
%06/01/2019
...
Ko = 0.0001;
    ...
    Kip     = data.Par(4);   %inhibition constant, inhibition of growth by product.
...
mu      = (qSox-qm).*Yem.*.75 + qsA.*Yxa + (qSof-pA).*Yxsof;
...
ly = {'q_S [g g^{-1} h^{-1}]','q_{Sof} [g g^{-1} h^{-1}]','q_{Sox} [g g^{-1} h^{-1}]','p_A [g g^{-1} h^{-1}]',...
  ...
  for k3 = 1:size(Celstate,2)
            ...
            %plot smooth lines without peaks
...
%            plot(T_plot,PKS,'LineWidth',1.5)
           ...
           %plot original oscillating profiles
           subplot(3,3,k3)
