function Volume_db_pulse = double_pulses(Specs,OD600,cultivation_age)  
...
OD600_db            = OD600(Specs.which_bioreactors_dbp);
number_reactors     = length(OD600_db);
mu_set              = Specs.mu_set;
X                   = abs(OD600_db.*0.37);                     % Biomass measured as OD600, convert to CDW. E coli BW25113 has conversion factor of 0.37
V                   = Specs.V_reactor./1000; 
V_db                = V(Specs.which_bioreactors_dbp);                % convert reactor volumes to L
tlog                = 10/3600;
Specs.T_vector(1)   = Specs.Pulse_interval.*Specs.T_vector(1);
Specs.T_vector(2)   = Specs.Pulse_interval.*Specs.T_vector(2);
Specs.pulses        = ceil(Specs.pulses/Specs.Pulse_interval);
...
Fe0                 = X.*V_db.*(mu_set./(data.Par(13)*Specs.Si));        % Calculate initial feed rate to maintain mu_set, L/h

%% Exponential Pulse fed-batch
Fp_db          = zeros(Specs.pulses,number_reactors);            % flow rate in case of pulse feed, L/h
V_pulse_db         = zeros(Specs.pulses,number_reactors);            % volume of pulses, what the pipette draws per feed cycle
X_next_db          = zeros(1,number_reactors);
Volumes_db         = zeros(Specs.pulses,number_reactors);            % Bioreactor volume in ml
Pulse_time_db      = zeros(Specs.pulses,1); 
w0_db              = zeros(number_reactors,6);                 % 6 is the number of state variables in y0
w0_db(:,[1;4;5])   = [X',100*ones(number_reactors,2)];         % Initial conditions
V_reactor_db       = Specs.V_reactor(Specs.which_bioreactors_dbp);
...
for k1  = 1:Specs.exponential_time*60/sum(Specs.T_vector);                                             % first 3hrs of pulses are done under exponential feeding.  
    ...
    V_pulse_db(k1,:) = Intfe.*(1000*1000);                 %give this volume in 1 minute, convert to uL.             % L
    Fp_db(k1,:)  = Intfe/deltsp;                       % calculate the corresponding feed flow rate in L/h, used only for the model solution.
  ...
  % Solve the model to get the new biomass to be used to calculate the pulse size, reconcile volumes and calculate dilution rates and new concentrations.
    ...
    for k2 = 1:number_reactors
        ...
        y0            = w0_db(k2,:);
        data.u(6)     = Fp_db(k1,k2);
        data.u(7)     = V_db(1,k2);                        % select which reactor to use
        ...
        w0_db(k2,:)      = y3(end,:);                      % set new parameters for looping.

        X_next_db(1,k2)  = y3(end,1);                      % new biomass concentration.

        V_reactor_db(1,k2) = V_reactor_db(1,k2) + V_pulse_db(k1,k2)./1e3 - Sample.*Specs.V_sample(1,k2);     %% Calculate the new volume after pulse addition and sampling. Sample is either 1 (for sampling) or 0 (no sampling), V_sample contains the sampling volumes of all 8 bioreacors

    end
    V_db                  = V_reactor_db./1000;
    Volumes_db(k1,:)      = V_db;
    X_new              = w0_db(:,1)';
    Fe0                = X_new.*V_db.*(mu_set./(data.Par(13)*Specs.Si));        % L/h
    T_start            = tse(end);
    Pulse_time_db(k1)    = tse(1);
end
...
%% Constant pulse, using the last value of the exponential feed as the constant pulse value
    ...
    for k3  = k1+1:Specs.pulses
        ...
        Fp_db(k3,:)  = Intfe./deltsp;                                   
        V_pulse_db(k3,:) = Intfe.*(1000*1000);                     
        for k4 = 1:number_reactors
            V_reactor_db(1,k4) = V_reactor_db(1,k4) + V_pulse_db(k3,k4)./1e3 - Sample.*Specs.V_sample(1,k4);        % Sample is either 1 (for sampling) or 0 (no sampling), V_sample contains the sampling volumes of all 8 bioreacors
            
        end
        V_db                   = V_reactor_db./1000;
        Volumes_db(k3,:)       = V_db; 
        Pulse_time_db(k3)     = Pulse_time_db(k3-1)+deltse;
        
%             if Sample == 1;
%                 volumes_ratio = Volumes_db(k3,:)./Volumes_db(k3-1,:);
...
%             end
    ...
    Pulse_time_db             = Pulse_time_db.*3600;

    Volume_db_pulse        = V_pulse_db;                           % Volume of feed to be pipetted (in microliters)
