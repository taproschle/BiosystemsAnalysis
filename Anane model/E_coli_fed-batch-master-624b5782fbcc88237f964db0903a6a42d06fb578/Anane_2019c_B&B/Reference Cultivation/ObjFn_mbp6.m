...
y0(6)       = data.U0; % initial glucose release rate
tspan21      = tspan(kla_change(1):kla_change(2));
options     = odeset('NonNegative',1:5);
[t21, y21]    = ode15s(@fn_Ecoli,tspan21,y0,options,data);
   
y0          = y21(end,:);

data.change = 6;
y0(6) = 0.6;
tspan22      = tspan(kla_change(2):kla_change(3));

[t22, y22]    = ode15s(@fn_Ecoli,tspan22,y0,options,data);
...
%% Constant pulse fed-batch: production
data.flag   = 2; 

tspan3      = tspan(kla_change(3):end);

[t3, y3]    = ode15s(@fn_Ecoli,tspan3,y0,options,data);
...
%         DOT_data = DOT_data(z_points);
        ...
        X_sim = Y(T_id(off_idx(1):off_idx(2)),1);           %locate sampling points in
        X_sim_normSc = wt(1).*(X_sim./(max(X_data)));          %normalized and scaled data

        S_sim = Y(T_id(off_idx(3):off_idx(4)),2);           %locate sampling points in
