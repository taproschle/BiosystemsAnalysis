...
for c = C:-1:1
    ...
    set(gca,'LineWidth',lineProps.width,'FontSize',FS,'FontWeight','bold') 
    ...
    %Make the cordinates for the patch
    yP=[lE,fliplr(uE)];
    xP=[x(c,:),fliplr(x(c,:))];
    ...
    H(c).edge(1)=plot(x(c,:),lE,'-','color',edgeColor1);
    H(c).edge(2)=plot(x(c,:),uE,'-','color',edgeColor1);
end
...
for c = C:-1:1
    ...
    H(c).edgeoverlap(1)=plot(x(c,:),lE,lineProps.edgestyle,'color',edgeColor1);
    H(c).edgeoverlap(2)=plot(x(c,:),uE,lineProps.edgestyle,'color',edgeColor1);
...
for c = 1:C
    ...
    set(get(get(H(c).edge(1),'Annotation'),'LegendInformation'),'IconDisplayStyle','on')
    set(get(get(H(c).edge(2),'Annotation'),'LegendInformation'),'IconDisplayStyle','on')
